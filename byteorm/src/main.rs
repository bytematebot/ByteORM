...
enum Commands {
    ...
    Push {
        ...
        /// Rewrite every generated file, even the ones already up to date
        #[arg(long)]
        force: bool,
    },
    ...
    match cli.command {
        ...
        Some(Commands::Push {
            ...
            if dry_run {
                ...
                if let Err(e) = generate_client_package(&schema, &project, force) {
...
fn generate_client_package(
    ...
    // A client generated by a different ByteORM is rewritten whole, even when
    // the schema has not moved: the runtime it embeds, the vendored macros and
    // the CLI have to come from one version.
    let current = env!("CARGO_PKG_VERSION");
    let previous = client_generator_version(&client_path);
    let upgraded = previous.as_deref() != Some(current);
    if upgraded && client_path.join("Cargo.toml").exists() {
        match previous.as_deref() {
            Some(old) => println!(
                "ByteORM {} -> {}: regenerating the whole client.",
                old, current
            ),
            None => println!(
                "Client was generated before ByteORM recorded versions; regenerating it whole."
            ),
        }
    }
    ...
    match project.dependency_source {
        DependencySource::Vendored => {
            let (macro_total, macro_rewritten) = embed_byteorm_macros(&client_path, force)?;
            total += macro_total;
            rewritten += macro_rewritten;
        }
    }

    let cargo_toml = generate_client_cargo_toml(&project.crate_name);
    total += 1;
    if write_file(&client_path.join("Cargo.toml"), &cargo_toml, force)? {
        rewritten += 1;
    }
    ...
    for (rel_path, content) in files {
        total += 1;
        if write_file(&client_path.join(rel_path), &content, force)? {
            rewritten += 1;
        }
    }
...
/// files keep their mtime and Cargo can treat the crate as fresh. `force`
/// writes regardless, which is what an upgrade or `--force` asks for.
fn write_file(path: &Path, content: &str, force: bool) -> Result<bool, Box<dyn std::error::Error>> {
    if !force
...
/// Returns how many vendored files were considered and how many were written.
fn embed_byteorm_macros(
    client_path: &Path,
    force: bool,
    ...
    for (rel, content) in embedded_macros_files() {
        total += 1;
        if write_file(&macros_path.join(rel), content, force)? {
            rewritten += 1;
        }
    }
    Ok((total, rewritten))
...
# Written by ByteORM. `byteorm generate` compares this against its own version
# and rewrites the whole client when they differ, so an upgraded CLI never
# leaves a client generated by an older one in place.
[package.metadata.byteorm]
generator_version = "{}"
...
/// The generator version recorded in an existing client, if any.
///
/// Parsed by hand rather than with a TOML crate: this reads one key out of a
/// file ByteORM itself wrote, and the rest of the CLI already parses its
/// config the same way.
fn client_generator_version(client_path: &Path) -> Option<String> {
    let content = fs::read_to_string(client_path.join("Cargo.toml")).ok()?;
    content
        .lines()
        .map(str::trim)
        .find_map(|line| line.strip_prefix("generator_version"))
        .and_then(|rest| rest.trim().strip_prefix('='))
        .and_then(|value| parse_toml_string(value.trim()))
}
